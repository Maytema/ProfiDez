...
function initFloatingWidget() {
    ...
    if (!floatingWidget || !widgetMinimized) return;
    ...
    if (floatingWidget) {
        floatingWidget.addEventListener('click', function(e) {
            if (e.target === floatingWidget || e.target.classList.contains('widget-btn')) {
                return;
            }
            e.stopPropagation();
        });
...
function initReviewForm() {
    ...
    const reviewFormData = document.getElementById('reviewFormData');
    if (reviewFormData) {
        reviewFormData.addEventListener('submit', function(e) {
            e.preventDefault();
